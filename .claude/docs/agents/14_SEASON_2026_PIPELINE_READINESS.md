...
- **Finals/grading:** CFB finals already land live (CFBD → `cfb_games`). **UPDATE 2026-06-18/27: NFL finals + signal/prop grading ARE now built** (`fill_finals.py`, `ingest_player_logs.py`, `grade_nfl_props`, `refresh_all_signal_performance`, `grade_week.sh` → `nfl-cfb-grade-daily` cron — see Cross-cutting gap #1). Still unbuilt: **AGENT-pick** grading for NFL/CFB (`grade-avatar-picks`, staged on PR #15).
...
- **Streaks / last-game / advanced-PBP / NGS / injuries / officials** — `nfl_pregame/{advanced_pbp,ngs,ftn,injuries,officials}.py` + `nfl_tr_scrape_nfl_schedules.py`.
  - **Injuries (2026-09-15):** the `nfl_pregame/injuries.py` path is dead (disabled workflow, broken loader, and nflverse's file only updates after the week's games). Live source is `espn_nfl_injuries.py` → `nfl_injuries_raw`, run before `fetch.py` in `run_nfl_week.sh` and on the game-day crons, so Friday designations and Sunday/primetime inactives reach the props table, `prop_rank`'s Out/Doubtful/Questionable-QB exclusion (Questionable QBs played 27% in 2024-25 vs ~60% for other skill positions; Out/Doubtful 0%), and the regression report before kickoff.
  - **Fantasy Points Data (2026-09-16):** paid FP Data Suite pulled through its own API — every readable tool in player / team-offense / team-defense scope, per week, 2021+ → `research/nfl-extreme-outcomes/data/fpdata/*.parquet` (full history) + Supabase `fp_data` (lean, current+prior season) via `fp_pull.py` / `fp_load.py`; Render `fp-data-inseason` keeps it current Tue/Thu. See `research/nfl-extreme-outcomes/FPDATA.md`.
...
- [ ] Extend `grade-avatar-picks` (AGENT picks) to NFL/CFB (doc 13) — staged on PR #15, not shipped.
- [ ] **Op hardening:** ensure the grade cron can run the direct-connection RPCs on Render — confirm `DATABASE_URL` is set in the env group AND `psql` is available, OR switch `grade_week.sh` steps 3-4 from `psql` to `psycopg2` (removes the binary dependency). Affects sides+props equally.
- [ ] **Live player-props feed:** `live_props.py` + Render cron `nfl-live-props-hourly` (BUILT 2026-06-27, in working tree, not yet committed) — writes `nfl_player_props` hourly so the prop signals have live lines to grade. See [[closing-line-definition]].
