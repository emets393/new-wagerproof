...
/// Ticket scope (`avatar_parlays.scope`): daily tickets surface only on their
/// `target_date`; weekly tickets are week-long NFL/CFB parlays that persist
/// through Monday night (surfaced by `week_key` in the snapshot RPC).
public enum AgentParlayScope: String, Codable, Sendable, Hashable {
    case daily
    case weekly
}
...
public struct AgentParlay: Codable, Identifiable, Sendable, Hashable {
    ...
    public let scope: AgentParlayScope
    ...
    public init(from decoder: Decoder) throws {
        ...
        // Tolerant: pre-migration rows / old RPC payloads have no scope column.
        self.scope = (try? c.decode(AgentParlayScope.self, forKey: .scope)) ?? .daily
...
public extension AgentParlay {
    ...
    /// Week-long ticket (persists through the football week's Monday night).
    var isWeekly: Bool { scope == .weekly }
