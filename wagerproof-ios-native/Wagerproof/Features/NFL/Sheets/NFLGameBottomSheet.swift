...
struct NFLGameBottomSheet: View {
    ...
    var body: some View {
        CollapsingWidgetScroll(
            heroMaxHeight: hasWeather ? 246 : 214,
            heroMinHeight: 124,
    ...
    private func heroView(progress p: CGFloat) -> some View {
        ...
        VStack(spacing: heroLerp(12, 6, p)) {
            ...
            HStack(alignment: .center, spacing: heroLerp(14, 10, p)) {
                heroTeamColumn(team: game.awayTeam, abbr: awayAbbr, colors: awayColors, size: logoSize, nameOpacity: detail, ml: game.awayMl, mlReveal: mlReveal)
                heroLinesColumn(detail: detail, p: p)
                heroTeamColumn(team: game.homeTeam, abbr: homeAbbr, colors: homeColors, size: logoSize, nameOpacity: detail, ml: game.homeMl, mlReveal: mlReveal)
            }
    ...
    @ViewBuilder
    private func heroTeamColumn(team: String, abbr: String, colors: TeamColorPair, size: CGFloat, nameOpacity: Double, ml: Int?, mlReveal: Double) -> some View {
    ...
    private func heroLinesColumn(detail: Double, p: CGFloat) -> some View {
        VStack(spacing: heroLerp(6, 2, p)) {
            ...
            if let score = game.predictedScore {
                heroLineRow(label: "Model", value: "\(awayAbbr) \(scoreText(score.away)) · \(homeAbbr) \(scoreText(score.home))", compact: true)
            }
        }
    ...
    @ViewBuilder
    private func heroLineRow(label: String, value: String, compact: Bool = false) -> some View {
        VStack(spacing: 2) {
