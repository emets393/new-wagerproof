...
/// Data-rich accuracy card used in the NBA Model Accuracy list screen.
/// Mirrors `wagerproof-mobile/components/nba/ModelAccuracyMatchupCard.tsx`:
/// a header with both team avatars + abbreviations + tipoff time, then
/// three labeled blocks (Spread / Moneyline / Over-Under), each with a
/// pick line + an accuracy row tinted by the historical hit rate.
///
/// Unlike the trends card, this one is non-tappable in RN (display-only).
struct NBAModelAccuracyMatchupCardView: View {
    let game: NBAModelAccuracyData
    ...
    private var header: some View {
        HStack(spacing: 8) {
            HStack(spacing: 6) {
                GameCardTeamAvatar(teamName: game.awayTeam, sport: "nba", size: 32)
                Text(game.awayAbbr)
                ...
                GameCardTeamAvatar(teamName: game.homeTeam, sport: "nba", size: 32)
                Text(game.homeAbbr)
                    .font(.system(size: 13, weight: .bold))
                    .foregroundStyle(Color.appTextPrimary)
            }
    ...
    /// Spread / ML / O/U row block. Renders the pick on the top line and the
    /// historical accuracy with sample size on the bottom line.
    @ViewBuilder
    private func pickBlock(label: String, pickValue: String, edgeValue: String?, accuracy: NBAAccuracyBucket?) -> some View {
        VStack(spacing: 4) {
            HStack {
                Text(label)
    ...
    @ViewBuilder
    private func accuracyText(_ acc: NBAAccuracyBucket?) -> some View {
        if let acc {
            HStack(spacing: 2) {
    ...
    private var spreadPickText: String {
        ...
        // RN flips the sign for the away team — show the cover line from
    ...
    /// Accuracy palette: green for trustworthy 60%+, yellow neutral, red fade.
    /// Matches RN's `getAccuracyColor` thresholds exactly.
