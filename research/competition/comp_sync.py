...
WINDOW_DAYS = 4          # deadline (Fri 12pm ET) .. Tue 12pm ET
HORIZON_DAYS = 9         # how far ahead we materialize games
DRY = "--dry" in sys.argv
...
def main() -> None:
    ...
    for sport, sport_key in SPORTS.items():
        ...
        for ev in events:
            ...
            n_elig += 1
        print(f"[{sport}] {len(events)} events, {n_elig} eligible in horizon")
