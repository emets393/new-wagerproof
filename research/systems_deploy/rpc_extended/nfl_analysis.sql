...
BEGIN
  ...
  -- Game totals must count each game once. Prefer the home row when both sides match;
  -- if team-perspective filters leave only away rows, DISTINCT ON still keeps those games.
  IF is_game_level THEN
    SELECT jsonb_build_object('n', count(*), 'wins', count(*) FILTER (WHERE hit=1),
      'hit_pct', round(avg(hit)::numeric*100,1),
      'roi', round((avg(hit)::numeric*1.909-1)*100,1))
      INTO overall FROM (
        SELECT DISTINCT ON (unique_id) hit FROM _f ORDER BY unique_id, is_home DESC
      ) d;
  ELSE
    SELECT jsonb_build_object('n', count(*), 'wins', count(*) FILTER (WHERE hit=1),
      'hit_pct', round(avg(hit)::numeric*100,1),
      'roi', CASE WHEN is_ml THEN NULL ELSE round((avg(hit)::numeric*1.909-1)*100,1) END)
      INTO overall FROM _f;
  END IF;
  IF is_game_level THEN
  ...
  IF is_game_level THEN
    SELECT jsonb_build_array(jsonb_build_object('dimension','over_under','options', jsonb_build_array(
      (SELECT jsonb_build_object('side','over','n',count(*),'wins',count(*) FILTER (WHERE hit=1),
         'hit_pct',round(avg(hit)::numeric*100,1),'roi',round((avg(hit)::numeric*1.909-1)*100,1))
         FROM (SELECT DISTINCT ON (unique_id) hit FROM _f ORDER BY unique_id, is_home DESC) d),
      (SELECT jsonb_build_object('side','under','n',count(*),'wins',count(*) FILTER (WHERE hit=0),
         'hit_pct',round((1-avg(hit))::numeric*100,1),'roi',round(((1-avg(hit))::numeric*1.909-1)*100,1))
         FROM (SELECT DISTINCT ON (unique_id) hit FROM _f ORDER BY unique_id, is_home DESC) d) )))
    INTO bars;
  ELSIF p_bet_type='team_total' THEN
    SELECT jsonb_build_array(jsonb_build_object('dimension','over_under','options', jsonb_build_array(
      (SELECT jsonb_build_object('side','over','n',count(*),'wins',count(*) FILTER (WHERE hit=1),
         'hit_pct',round(avg(hit)::numeric*100,1),'roi',round((avg(hit)::numeric*1.909-1)*100,1)) FROM _f),
      (SELECT jsonb_build_object('side','under','n',count(*),'wins',count(*) FILTER (WHERE hit=0),
         'hit_pct',round((1-avg(hit))::numeric*100,1),'roi',round(((1-avg(hit))::numeric*1.909-1)*100,1)) FROM _f) )))
    INTO bars;
  ELSE
