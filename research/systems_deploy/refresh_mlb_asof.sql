...
h2h AS (
  SELECT game_pk, team_abbr,
    ...
    lag(is_home)     OVER wh AS h2h_last_home,
    lag(is_favorite) OVER wh AS h2h_last_fav,
    lag(rl_covered)  OVER wh AS h2h_last_rl_cover
  FROM mlb_analysis_base
  WINDOW wh AS (PARTITION BY team_abbr, opponent_abbr ORDER BY game_date, time_et, game_pk)
),
-- previous-game flags (cross-season carry, matching prev_result's convention).
-- These columns were NULL since the parity build — the last-favorite / last-RL /
-- last-over filters silently returned zero games.
lastflags AS (
  SELECT game_pk, team_abbr,
    lag(is_favorite) OVER wl AS last_is_favorite,
    lag(rl_covered)  OVER wl AS last_rl_covered,
    lag(ou_over)     OVER wl AS last_ou_over
  FROM mlb_analysis_base
  WINDOW wl AS (PARTITION BY team_abbr ORDER BY game_date, time_et, game_pk)
),
teamvals AS (
  SELECT c.game_pk, c.team_abbr, c.opponent_abbr, c.season,
    ...
    h.h2h_last_home, h.h2h_last_fav, h.h2h_last_rl_cover,
    lf.last_is_favorite, lf.last_rl_covered, lf.last_ou_over,
    c.prev_result AS own_prev_result, c.prev_margin AS own_prev_margin
  FROM cum c
  LEFT JOIN lastflags lf ON lf.game_pk = c.game_pk AND lf.team_abbr = c.team_abbr
...
SELECT t.*,
  ...
  o.last_is_favorite AS opp_last_is_favorite, o.last_rl_covered AS opp_last_rl_covered,
  o.last_ou_over AS opp_last_ou_over
FROM teamvals t
...
UPDATE mlb_analysis_base b SET
  ...
  h2h_last_home = a.h2h_last_home, h2h_last_fav = a.h2h_last_fav,
  h2h_last_rl_cover = a.h2h_last_rl_cover,
  last_is_favorite = a.last_is_favorite, last_rl_covered = a.last_rl_covered,
  last_ou_over = a.last_ou_over,
  opp_last_is_favorite = a.opp_last_is_favorite, opp_last_rl_covered = a.opp_last_rl_covered,
  opp_last_ou_over = a.opp_last_ou_over
FROM _asof a
