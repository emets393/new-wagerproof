...
BEGIN
  ...
  WHERE
    ...
    AND (p_filters->'series_game_in' IS NULL OR b.series_game::text IN (SELECT jsonb_array_elements_text(p_filters->'series_game_in')))
    ...
    AND (p_filters->>'sp_era_min' IS NULL OR b.sp_season_era >= (p_filters->>'sp_era_min')::numeric)
    AND (p_filters->>'sp_era_max' IS NULL OR b.sp_season_era <= (p_filters->>'sp_era_max')::numeric)
    AND (p_filters->>'opp_sp_era_min' IS NULL OR b.opp_sp_season_era >= (p_filters->>'opp_sp_era_min')::numeric)
    AND (p_filters->>'opp_sp_era_max' IS NULL OR b.opp_sp_season_era <= (p_filters->>'opp_sp_era_max')::numeric)
