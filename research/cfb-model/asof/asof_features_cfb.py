"""As-of-game feature aggregation for cfb_analysis_base (Systems feature, Bucket C).
...
Source & sink: warehouse project jpxnjuwglavsjbgbasnl, table cfb_analysis_base (grain = one row
per (game_id, team), team names = full school names). We read the base itself so signs/results
match the live data exactly (no fragile external join). Output = asof_cfb.parquet (merge frame,
...
OUT = Path(__file__).resolve().parent / "asof_cfb.parquet"

SEL = ("game_id,season,week,team,opponent,is_home,is_favorite,team_score,opp_score,"
       "total_points,fg_won,fg_covered,ou_result,fg_spread,fg_total,team_ml,game_type")
...
def fetch_all(k):
    ...
    for off in range(0, 100000, step):
        req = urllib.request.Request(f"{BASE}/cfb_analysis_base?select={SEL}&order=season.asc,week.asc,game_id.asc"
...
def add_prior_year(df):
    season_tot = (df.groupby(["team", "season"])
                    .agg(wins=("fg_won", lambda s: (s == 1).sum()),
                         ...
                         playoff=("game_type", lambda s: s.isin(["bowl", "playoff"]).any()))
                    .reset_index())
...
def main():
    k = key()
    print("fetching cfb_analysis_base ...")
    df = fetch_all(k)
