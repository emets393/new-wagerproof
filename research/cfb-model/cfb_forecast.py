...
def main():
    ...
    if os.path.exists(evp):
        ...
        # ===== 1H MODELS (validated): NOSTR spread (53.7% conf) + pruned-15 tempo totals (55.8U/54.8O shop)
        import h1_signals
        nets_ = [c for c in gm.columns if c.startswith("net_")]
        hs1 = h1_signals.build(gm, feats, nets_, a.season)
        # posted 1H spread consensus (tag-agnostic close) + total best lines
        sp1 = pd.read_parquet(evp)
        sp1 = sp1[sp1.market == "spreads_h1"].copy()
        sp1["nm"] = sp1.name.map(_tdb); sp1 = sp1[sp1.nm == sp1.home]
        sp1 = sp1.merge(kicks.rename(columns={"id": "game_id"}), on="game_id", how="left")
        sp1["hrs"] = (pd.to_datetime(sp1.startDate, utc=True) - pd.to_datetime(sp1.snap, utc=True)).dt.total_seconds() / 3600
        sp1 = sp1[sp1.hrs > 0]; sp1["minh"] = sp1.groupby(["season", "game_id", "book"]).hrs.transform("min")
        spc = sp1[sp1.hrs == sp1.minh].groupby(["season", "game_id"]).point.median().rename("hs").reset_index()
        t1b = h1[h1.hrs == h1.minh].groupby(["season", "game_id"]).point.agg(h1t_hi="max", h1t_lo="min").reset_index() if "minh" in h1 else None
        B = hs1.merge(spc, on=["season", "game_id"], how="left")
        if t1b is not None: B = B.merge(t1b, on=["season", "game_id"], how="left")
        B = B.merge(te[["season", "game_id", "homeTeam", "awayTeam"]], on=["season", "game_id"], how="left")
        B["h1_spread_edge"] = B.h1_pm + B.hs
        B["h1_spread_bet"] = np.where(B.h1_spread_edge.abs() >= 3, np.where(B.h1_spread_edge > 0, "HOME", "AWAY"), "")
        B["h1_tot_bet"] = ""
        if "h1t_hi" in B:
            B.loc[B.h1_pt <= B.h1t_hi - 2, "h1_tot_bet"] = "UNDER@" + B.h1t_hi.astype(str)
            B.loc[B.h1_pt >= B.h1t_lo + 2, "h1_tot_bet"] = "OVER@" + B.h1t_lo.astype(str)
        # 1H ML dog-conversion (h1_ml_conv.py): NOSTR predicts ML-dog WINS half by >=2 -> bet dog ML @ best
        mlq = pd.read_parquet(evp); mlq = mlq[mlq.market == "h2h_h1"].copy()
        mlq["t"] = mlq.name.map(_tdb)
        mh_ = mlq[mlq.t == mlq.home].groupby(["season", "game_id"]).price.max().rename("mlh_best").reset_index()
        ma_ = mlq[mlq.t == mlq.away].groupby(["season", "game_id"]).price.max().rename("mla_best").reset_index()
        B = B.merge(mh_, on=["season", "game_id"], how="left").merge(ma_, on=["season", "game_id"], how="left")
        B["h1_ml_bet"] = ""
        B.loc[(B.h1_pm >= 2) & (B.mlh_best > 0) & (B.mlh_best <= 1200), "h1_ml_bet"] = "HOME ML"
        B.loc[(B.h1_pm <= -2) & (B.mla_best > 0) & (B.mla_best <= 1200), "h1_ml_bet"] = "AWAY ML"
        bb = B[(B.h1_spread_bet != "") | (B.h1_tot_bet != "") | (B.h1_ml_bet != "")]
        bb.to_csv(os.path.join(OUT, f"cfb_h1_model_{a.season}.csv"), index=False)
        gg2 = bb[bb.h1m.notna()]
        if len(gg2):
            sb = gg2[gg2.h1_spread_bet != ""]; sb = sb[(sb.h1m + sb.hs) != 0]
            sw = np.where(sb.h1_spread_bet == "HOME", (sb.h1m + sb.hs) > 0, (sb.h1m + sb.hs) < 0)
            print(f"  1H MODEL: spread n={len(sb)} hit {100*sw.mean() if len(sb) else 0:.1f}% | {len(bb)} bets -> cfb_h1_model_{a.season}.csv")
