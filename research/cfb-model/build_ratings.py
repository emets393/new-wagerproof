...
def _seasons_in_cache(pattern):
    ...
    fetch_cfbd.py lands their parquets (no annual code edit). 2020 (COVID) stays
    deliberately excluded — an anomalous season that degraded the model."""
    yrs = set()
    for p in glob.glob(os.path.join(DATA, pattern)):
        tok = os.path.basename(p).split("_")[-1].split(".")[0]
        if tok.isdigit():
            yrs.add(int(tok))
    return sorted(y for y in yrs if y != 2020)


YEARS = _seasons_in_cache("game_advanced_*.parquet") or \
