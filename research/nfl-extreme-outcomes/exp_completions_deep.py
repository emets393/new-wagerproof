...
import io, contextlib, glob, itertools, numpy as np, pandas as pd, warnings
...
d, F0 = prep2("player_pass_completions", ["QB"], None); d = d[d.close_line.notna() & d.actual.notna()].copy()
d["team"] = d.team.replace({"LAR": "LA"}); d["opp"] = d.opp.replace({"LAR": "LA"})
print(f"completions frame: {len(d)} QB-games 2023-25, {len(F0)} prop-engine features")
...
# ================================================================= QB per-play splits x opponent tendency
COLS = ["game_id","play_id","season","week","posteam","defteam","qb_dropback","qb_scramble","complete_pass","pass_attempt","passer_player_name","rusher_player_name","sack","season_type"]
...
pbp = pbp[(pbp.season_type == "REG") & (pbp.pass_attempt == 1) & (pbp.sack != 1)]
...
ftn["play_id"] = num(ftn.play_id); ftn = ftn[["game_id","play_id","n_blitzers"]].drop_duplicates(["game_id","play_id"])
par = pd.concat([pd.read_parquet(f"data/pbp_participation_{y}.parquet", columns=["nflverse_game_id","play_id","was_pressure","defense_man_zone_type"]) for y in (2022, 2023, 2024, 2025)]).rename(columns={"nflverse_game_id":"game_id"}).drop_duplicates(["game_id","play_id"])
P = pbp.merge(ftn, on=["game_id","play_id"], how="left").merge(par, on=["game_id","play_id"], how="left")
P["blitz"] = (num(P.n_blitzers) >= 1).where(P.n_blitzers.notna()).astype(float); P["man"] = P.defense_man_zone_type.map({"MAN_COVERAGE": 1.0, "ZONE_COVERAGE": 0.0}).astype(float); P["press"] = P.was_pressure.map({True: 1.0, False: 0.0}).astype(float); P["cp"] = num(P.complete_pass).astype(float)
P["qb"] = P.passer_player_name; P = P[P.qb.notna()]
def split_rates(g):
    o = {}
    for lab, mask in (("blitz", g.blitz == 1), ("noblitz", g.blitz == 0), ("man", g.man == 1), ("zone", g.man == 0), ("press", g.press == 1), ("clean", g.press == 0)):
        s = g[mask]; o["n_" + lab] = len(s); o["cp_" + lab] = s.cp.sum()
    return pd.Series(o)
QW = P.groupby(["qb","season","week"]).apply(split_rates).reset_index().sort_values(["qb","season","week"])
...
for lab in ("blitz","noblitz","man","zone","press","clean"): QW["e_cp_" + lab] = entering_rate(QW, "qb", "n_" + lab, "cp_" + lab, 40)
QW["sens_blitz"] = QW.e_cp_blitz - QW.e_cp_noblitz; QW["sens_man"] = QW.e_cp_man - QW.e_cp_zone; QW["sens_press"] = QW.e_cp_press - QW.e_cp_clean
DW = P.groupby(["defteam","season","week"]).agg(n=("cp","size"), nb=("blitz","sum"), nm=("man","sum"), npr=("press","sum"), n_b=("blitz","count"), n_m=("man","count"), n_p=("press","count")).reset_index().rename(columns={"defteam":"opp"}).sort_values(["opp","season","week"])
for c in ("nb","nm","npr","n_b","n_m","n_p"): DW[c] = num(DW[c]).astype(float)
for c in [c for c in QW.columns if c.startswith(("n_","cp_"))]: QW[c] = num(QW[c]).astype(float)
for lab, nc, cc in (("blitz","n_b","nb"), ("man","n_m","nm"), ("press","n_p","npr")): DW["opp_rate_" + lab] = entering_rate(DW, "opp", nc, cc, 120)
d["qbkey"] = d.player_name.map(lambda s: (s.split()[0][0] + "." + " ".join(s.split()[1:])) if " " in str(s) else s)
# the pbp short name drops suffixes and uses the last token; match on first initial + last name
qk = {q: q for q in QW.qb.unique()}
def match_qb(nm):
    nm = str(nm)
    if nm in qk: return nm                                   # panel names are already pbp-style ("J.Daniels")
    parts = nm.replace(".", " ").split(); cand = parts[0][0] + "." + parts[-1] if parts else nm
    if cand in qk: return cand
    alt = [q for q in qk if q.split(".")[-1].lower() == parts[-1].lower() and q[0].lower() == parts[0][0].lower()] if parts else []
    return alt[0] if alt else None
d["qb"] = d.player_name.map(match_qb)
d = d.merge(QW[["qb","season","week","e_cp_blitz","e_cp_noblitz","e_cp_man","e_cp_zone","e_cp_press","e_cp_clean","sens_blitz","sens_man","sens_press"]], on=["qb","season","week"], how="left")
d = d.merge(DW[["opp","season","week","opp_rate_blitz","opp_rate_man","opp_rate_press"]], on=["opp","season","week"], how="left")
d["ix_sens_blitz"] = d.sens_blitz * (d.opp_rate_blitz - DW.opp_rate_blitz.mean()); d["ix_sens_man"] = d.sens_man * (d.opp_rate_man - DW.opp_rate_man.mean()); d["ix_sens_press"] = d.sens_press * (d.opp_rate_press - DW.opp_rate_press.mean())
QBS = ["e_cp_blitz","e_cp_noblitz","e_cp_man","e_cp_zone","e_cp_press","e_cp_clean","sens_blitz","sens_man","sens_press","opp_rate_blitz","opp_rate_man","opp_rate_press","ix_sens_blitz","ix_sens_man","ix_sens_press"]
print(f"  QB split coverage: {d.e_cp_blitz.notna().mean():.0%} | receivers {d.e_rw_catchable.notna().mean():.0%} | context {d.wind.notna().mean():.0%}")
# ================================================================= families
QBF = [c for c in F0 if c.startswith(("qb_","ix_press","ix_hold","ix_scram","ix_chk","ix_qb"))]; TEAM = [c for c in F0 if c.startswith(("off_","T_","tm_","ix_script_pass"))]; OPP = [c for c in F0 if c.startswith(("def_","O_","OL_","oppallow"))]
FORM = [c for c in ("l3","l5","szn") if c in d.columns]; LINE = ["close_line"]
FAM = {"line": LINE, "form": FORM, "qb": QBF, "team": TEAM, "opponent": OPP, "receivers": RECV, "context": CONTEXT, "qb_splits": QBS, "injury": CTX}
...
print("\n" + "=" * 118); print("(1) ACCURACY by cumulative feature stack — walk-forward (train < season). MAE in completions; r = corr(pred, actual). Line-only = predict the line."); print("=" * 118)
stacks = [("line only", LINE)]; acc = LINE
for name in ("form","qb","team","opponent","receivers","context","qb_splits","injury"): acc = acc + FAM[name]; stacks.append(("+ " + name, list(acc)))
...
for name, F in stacks:
    ...
    for yr in (2024, 2025):
        ...
        w, n, roi = gbest(te, pred, 1.75); out.append(f"{yr}: {np.abs(pred - te.actual).mean():4.2f} {np.corrcoef(pred, te.actual)[0,1]:+.2f} | {100*w if n else np.nan:4.1f}%/{n:3d}")
...
print("\n" + "=" * 118); print("(2) WHICH FAMILIES MOVE COMPLETIONS — drop-one from the full stack (2025 holdout, train 2023-24): change in MAE (+ = family helped)"); print("=" * 118)
...
print("\n" + "=" * 118); print("(3) PARTIAL EFFECTS — actual completions minus the posted line, by bucket (pooled 2023-25). The line already prices what it prices; a non-zero row is what it misses."); print("=" * 118)
...
bucket("temp", [-99, 32, 50, 75, 199], ["<=32","32-50","50-75","75+"]); bucket("div_game", [-1, 0.5, 1.5], ["non-div","divisional"]); bucket("rest", [0, 5, 8, 99], ["short","normal","long/bye"]); bucket("opp_rate_blitz", [-1, 0.22, 0.30, 1], ["low blitz","mid","high blitz"]); bucket("opp_rate_man", [-1, 0.22, 0.32, 1], ["zone-heavy","mid","man-heavy"])
bucket("e_rw_catchable", [-1, 0.72, 0.78, 2], ["low catchable","mid","high catchable"]); bucket("inj_wrte_tgt_out", [-1, 0.001, 0.15, 2], ["no WR/TE out","<15% share out","15%+ share out"])
...
res = []
for lam, thr, (sname, F) in itertools.product((60, 200, 600), (1.0, 1.75, 2.5), [("current (engine SETS)", [c for c in PE.SETS["player_pass_completions"] if c in d.columns]), ("full stack", ALL), ("full minus context", [c for c in ALL if c not in CONTEXT]), ("full + hgb", ALL)]):
    r = {}
    for yr in (2024, 2025):
...
x = R[(R["stack"] == "current (engine SETS)") & (R.lam == 60) & (R.thr == 1.75)].iloc[0]; print(f"  frozen config (λ60, thr 1.75, engine SETS): 2024 {100*x.w24:.1f}%/{x.n24} ROI {100*x.roi24:+.1f}% | 2025 {100*x.w25:.1f}%/{x.n25} ROI {100*x.roi25:+.1f}%")
x = R[(R["stack"] == "full stack") & (R.lam == 200) & (R.thr == 1.75)].iloc[0]; print(f"  full stack (λ200, thr 1.75):                 2024 {100*x.w24:.1f}%/{x.n24} ROI {100*x.roi24:+.1f}% | 2025 {100*x.w25:.1f}%/{x.n25} ROI {100*x.roi25:+.1f}%")
d.to_parquet("data/_completions_deep_frame.parquet", index=False)
