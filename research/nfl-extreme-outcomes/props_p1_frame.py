...
# Frame seasons follow the props archive on disk (2023 backfill added 2026-08-17).
...
def main():
    ...
    pr["snapshot_time"] = pd.to_datetime(pr.snapshot_time, utc=True)
    pr["commence_time"] = pd.to_datetime(pr.commence_time, utc=True)
    pr["gameday"] = pr.commence_time.dt.date

    po = pd.read_parquet(ROOT / "data" / "player_offense.parquet")
    po = po[po.season.isin(SEASONS)].copy()
    ...
    ge = ge[ge.season.isin(SEASONS)][["season", "week", "home_team", "away_team"]]
    ...
    inj = inj[inj.season.isin(SEASONS)]
