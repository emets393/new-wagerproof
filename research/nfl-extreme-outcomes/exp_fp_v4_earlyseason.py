...
print(f"panel {len(p)} | V4 (prior-seeded fits+composites), FULL feature set, wk1+:")
print("SPREAD thr>=2 by week bucket:")
for lo, hi, lab in ((1, 3, "wk1-3"), (4, 6, "wk4-6"), (7, 11, "wk7-11"), (12, 18, "wk12-18"), (1, 18, "ALL")):
    d = gg[(gg.week >= lo) & (gg.week <= hi) & (gg.e.abs() >= 2) & ((gg.margin + gg.line) != 0)]
    pk = d.e >= 2; w = np.where(pk, (d.margin + d.line) > 0, (d.margin + d.line) < 0)
    print(f"  {lab:9s}: {w.sum()}-{len(d)-w.sum()} ({100*w.mean():.0f}%) n={len(d)}" if len(d) else f"  {lab}: n=0")
