...
for gid, x in GD.items():
    ...
    for r in fp[fp.hc.fillna(False).astype(bool)].itertuples():
        ...
        own = f" His own homecomings: {int(r.hc_games)} games, over the line {100*r.hc_over:.0f}% (elsewhere {100*r.else_over:.0f}%)." if pd.notna(getattr(r, "hc_games", np.nan)) else " First homecoming on record."
        read = " Quarterbacks visiting near home have gone under 63% of the time (54 lines, 2023-25 — small)." if r.position == "QB" else " As a group, visiting players near home go over 51% — the book prices it; his own record is the tell."
...
for gid, x in GD.items():
    ...
    for team, nm in ((x["away"], x["an"]), (x["home"], x["hn"])):
        ...
        if pc and pc in ID.index:
            ...
            gl = ("rides his No. 1 back at the goal line" if pd.notna(i.rb1_share_in5) and i.rb1_share_in5 - i.rb1_share_all >= 0.08 else "subs the spell back at the goal line" if pd.notna(i.rb1_share_in5) and i.rb1_share_in5 - i.rb1_share_all <= -0.08 else "keeps the same back split at the goal line")
            L.append(f"### 🧠 {nm} — {pc} ({'head coach, calls the plays' if role == 'HC' else 'offensive coordinator'})")
            L.append(f"- **Identity** ({int(i.plays):,} plays 2022-25): pass rate over expected **{i.proe:+.1f}** (0 = league); early-down neutral pass {pct(i.early_neutral_pass)}; motion {pct(i.motion)}, play-action {pct(i.play_action)} of passes, no-huddle {pct(i.no_huddle)}; goes for it on 4th-and-short {pct(i.fourth_go)} (n={int(i.n_fourth)})")
            ...
            L.append(f"- **Goal-line back**: lead back gets {pct(i.rb1_share_all)} of carries overall, {pct(i.rb1_share_in10)} inside the 10, {pct(i.rb1_share_in5)} inside the 5 — **{gl}**" + (f"; lead back this week by the rushing line: {lead_rb[team]}" if team in lead_rb else "") + (f"; QB designed runs {pct(i.qb_run_share_in5)} of inside-5 runs" if pd.notna(i.qb_run_share_in5) and i.qb_run_share_in5 >= 0.2 else ""))
            for r in on: L.append(f"- **{r.situation.capitalize()} — live this week**: he passes {pct(r.pass_rate)} here (league {pct(r.lg_pass_rate)}), {r.relative:+.0f} pts more {'pass' if r.relative > 0 else 'run'}-heavy than the league's own shift in this spot, every season ({r.seasons.replace(':', ' ')}; n={int(r.plays)})")
            if on: summ.append(f"{pc} ({team}) {'throws' if on[0].relative > 0 else 'runs'} more than the league in {on[0].situation.split(' (')[0]} games, every season")
            elif abs(i.proe) >= 3: summ.append(f"{pc} ({team}) calls it {'pass' if i.proe > 0 else 'run'}-heavy (pass rate {abs(i.proe):.1f} points {'above' if i.proe > 0 else 'below'} what the league would call in the same spots) and {gl}")
        ...
        if dc and dc in DID.index:
            ...
            L.append(f"- **Identity** ({int(d.dropbacks):,} charted dropbacks 2022-25): blitz {pct(d.blitz)} (league {pct(LGI['blitz'])}), 6+ rushers {pct(d.rush6)}; heavy box on runs {pct(d.heavy_box)}, stacked {pct(d.stacked)}; blitz on 3rd-and-long {pct(d.blitz_3rd_long)}, in the red zone {pct(d.blitz_rz)}, in two-minute {pct(d.blitz_two_min)}")
            for r in don: L.append(f"- **{r.situation.capitalize()} — live this week**: blitzes {pct(r.blitz)} here (league {pct(r.lg_blitz)}), {100*r.relative:+.0f} pts vs the league's shift, every season ({r.seasons.replace(':', ' ')}; n={int(r.dropbacks)})")
            if abs(d.blitz - LGI["blitz"]) >= 0.07: summ.append(f"{dc} ({team}) blitzes {pct(d.blitz)} against a league {pct(LGI['blitz'])}")
...
for gid, x in GD.items():
    ...
    if L: add("player_tendencies", f"tend:{gid}", gid, f"Player tendencies live this week — {x['label']}", (first + f". {len(L)} players with a tendency that holds every season and is triggered this week." if first else ""), "### 📈 Tendencies that hold every season, triggered this week\n" + "\n".join(L), "Each player's own priced lines 2023-25 (best of four books), split by the factor; a tendency is listed only when its sign holds in every season with 8+ games. Opponent blitz rate = the coordinator's 2022-25 rate.", 22)
