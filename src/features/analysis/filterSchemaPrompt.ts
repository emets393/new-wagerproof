...
export function buildNflSystemPrompt(spec: DimensionSpec[] = buildNflDimensionSpec()): string {
  ...
  lines.push(
    ...
    '16. FAVORITES / UNDERDOGS with NO point number ("favorites", "home dogs", "chalk", "road',
    '   underdogs"): on bet types fg_ml, h1_ml, team_total set favDog ("favorite"/"underdog"); on the',
    '   spread bet types set spreadSide instead. WITH a number ("favored by 7+") ALWAYS use spreadSide +',
    '   spreadSize regardless of bet type. Never set BOTH favDog and spreadSide for the same phrase.',
    '17. HOME/AWAY does not apply on game totals (betType fg_total / h1_total) — a game has no side there.',
    '   If the user asks for home/away while on a game-total market (and is not switching markets),',
    '   couldnt_map it with a one-line reason instead of setting "side".',
    '18. MARKET WORDS AS THE SUBJECT — "show me unders", "overs", "totals": that is a MARKET switch →',
    '   betType fg_total (the page shows the over AND under records side by side). Do NOT translate the',
    '   word "unders" alone into overPct or underStreak. Same for "team totals" → betType team_total.',
    '19. RESET — "reset", "clear everything", "start over", "blank slate": emit a "clear" op for EVERY',
    '   dimension present in CURRENT FILTER (except betType unless they name a market). Do not guess new',
    '   filters after a reset ask.',
    '20. NFL HAS NO MONTH FILTER — approximate months with regular-season weeks and say so via the weeks',
    '   dim: September→[1,4], October→[4,9], November→[9,13], December→[13,18], January (regular',
    '   season)→[17,18]. "December games" → seasonType="regular" + weeks=[13,18].',
    '21. GLOSSARY — "a touchdown" = 7 points, "a field goal" = 3. "Off a bye" → restBye="off_bye";',
    '   "week before a bye" → restBye="pre_bye"; "short week"/"on short rest"/"Thursday after Sunday" →',
    '   restBye="short".',
    '',
    'EXAMPLES:',
    ...
    '',
    'CURRENT FILTER: {"betType":"fg_ml"}',
    'REQUEST: home favorites',
    'OUTPUT: {"ops":[{"op":"set","dimension":"side","value":"home"},{"op":"set","dimension":"favDog","value":"favorite"}],"couldnt_map":[],"ambiguous":[]}',
    '',
    'CURRENT FILTER: {"betType":"fg_spread"}',
    'REQUEST: unders in December for teams off a bye',
    'OUTPUT: {"ops":[{"op":"set","dimension":"betType","value":"fg_total"},{"op":"set","dimension":"seasonType","value":"regular"},{"op":"set","dimension":"weeks","value":[13,18]},{"op":"set","dimension":"restBye","value":"off_bye"}],"couldnt_map":[],"ambiguous":[]}',
    '',
    'CURRENT FILTER: {"betType":"fg_total","lineRange":[47,60],"dome":true}',
    'REQUEST: start over',
    'OUTPUT: {"ops":[{"op":"clear","dimension":"lineRange","value":null},{"op":"clear","dimension":"dome","value":null}],"couldnt_map":[],"ambiguous":[]}',
    '',
    'CURRENT FILTER: {"betType":"fg_total"}',
    'REQUEST: home teams in games totaling 47 or more',
    'OUTPUT: {"ops":[{"op":"set","dimension":"lineRange","value":[47,60]}],"couldnt_map":["home/away — a full-game total has no team side; switch to a spread/ML/team-total market to filter by side"],"ambiguous":[]}',
    '',
    'CURRENT FILTER: {"betType":"fg_spread"}',
