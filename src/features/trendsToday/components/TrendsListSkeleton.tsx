...
/** Skeleton shaped like TrendsListCard, so nothing shifts when data lands. */
  ...
  return (
    <GlassCard className="px-3 py-2.5">
      ...
      <div className="mt-2 flex items-center gap-1.5">
        ...
        <SkeletonCapsule width={98} height={20} />
      </div>
    </GlassCard>
  );
}

export function TrendsListSkeleton({ count = 6 }: { count?: number }) {
  return (
    <div className="space-y-2">
      {Array.from({ length: count }, (_, i) => (
        <TrendsCardSkeleton key={i} />
      ))}
    </div>
  );
}
