...
>((props, ref) => {
  const {
    texts,
    transition = { type: "spring", damping: 25, stiffness: 300, layout: { duration: 0.35, ease: "easeInOut" } },
  ...
  return (
    <motion.span className={cn("text-rotate", mainClassName, stabilizeWidth && "relative")} {...(rest as any)} layout transition={transition}>
      <span className="text-rotate-sr-only">{texts[currentTextIndex]}</span>
      {stabilizeWidth && (
        <span aria-hidden="true" className={cn(splitBy === "lines" ? "text-rotate-lines" : "text-rotate", "invisible")}>{longestText}</span>
      )}
      <span className={stabilizeWidth ? "absolute left-0 top-0" : undefined} aria-hidden="true">
