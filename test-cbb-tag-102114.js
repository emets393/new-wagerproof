...
async function testCBBTag102114() {
  console.log('\n' + '='.repeat(70));
  console.log('🏀 TESTING CBB WITH tag_id=102114');
  ...
  try {
    ...
    if (games.length === 0) {
      console.log('ℹ️  No active CBB games found');
      console.log('   This could mean:');
      console.log('   - No games scheduled right now');
      console.log('   - It\'s off-season');
      console.log('   - Games haven\'t been posted yet\n');
      
      // Try with closed=true to see if there are any games at all
      console.log('Checking for closed games...\n');
      const closedUrl = `${BASE_URL}/events?tag_id=${tagId}&limit=20`;
      const closedResponse = await fetch(closedUrl);
      if (closedResponse.ok) {
        const closedData = await closedResponse.json();
        const closedEvents = Array.isArray(closedData) ? closedData : (closedData.events || []);
        const closedGames = closedEvents.filter(event => {
          const title = event.title || event.question || '';
          return parseTeamsFromTitle(title) !== null;
        });
        
        if (closedGames.length > 0) {
          console.log(`📋 Found ${closedGames.length} closed CBB games:`);
          closedGames.slice(0, 5).forEach((game, idx) => {
            const teams = parseTeamsFromTitle(game.title);
            console.log(`\n${idx + 1}. ${game.title}`);
            if (teams) {
              console.log(`   ${teams.awayTeam} @ ${teams.homeTeam}`);
            }
    ...
    // Display games
    console.log('='.repeat(70));
    console.log(`🏀 CBB GAMES (${games.length} active)`);
    console.log('='.repeat(70) + '\n');
    ...
    console.log('='.repeat(70));
    console.log('✅ Summary: Found ' + games.length + ' active CBB games');
    console.log('='.repeat(70) + '\n');
