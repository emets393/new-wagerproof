...
export function NBAModelAccuracyMatchupCard({ game }: Props) {
  const theme = useTheme();
  const { isDark } = useThemeContext();

  const awayColors = getNBATeamColors(game.awayTeam);
  const homeColors = getNBATeamColors(game.homeTeam);
    ...
    <View style={[styles.card, { backgroundColor: isDark ? '#1a1a1a' : '#ffffff' }]}>
      ...
      <View style={styles.header}>
        <View style={styles.teamRow}>
          <TeamAvatar teamName={game.awayTeam} sport="nba" size={32} teamAbbr={game.awayAbbr} />
          ...
          <TeamAvatar teamName={game.homeTeam} sport="nba" size={32} teamAbbr={game.homeAbbr} />
