...
export function NBAGameBottomSheet() {
  ...
  const spreadPrediction = useMemo(() => {
    ...
    if (game.home_away_spread_cover_prob !== null) {
      ...
      return {
        edge: (prob - 0.5) * 20, // Convert probability to edge-like value
  ...
  const ouPrediction = useMemo(() => {
    ...
    if (game.model_fair_total !== null && game.over_line !== null) {
      ...
      const isOver = game.model_fair_total > game.over_line;
      return {
        edge,
              ...
              >
                <View style={[styles.sectionCard, { backgroundColor: isDark ? '#222' : '#f5f5f5', borderColor: isDark ? '#333' : '#e0e0e0' }]}>
                  ...
                  <View style={styles.edgeSummaryRow}>
                    <TeamAvatar teamName={spreadPrediction.predictedTeam} sport="nba" size={40} />
                    <View style={styles.edgeSummaryText}>
                      <Text style={[styles.edgeSummaryTeam, { color: theme.colors.onSurface }]}>
                        Edge to {spreadPrediction.predictedTeam}
                      </Text>
            ...
            <View style={[styles.sectionCard, { backgroundColor: isDark ? '#222' : '#f5f5f5', borderColor: isDark ? '#333' : '#e0e0e0' }]}>
              ...
              <View style={[styles.statsContent, { backgroundColor: 'rgba(255, 255, 255, 0.05)' }]}>
                <View style={styles.statsHeaderRow}>
                  ...
                  <Text style={[styles.statsHeaderValue, { color: theme.colors.onSurfaceVariant }]}>{getNBATeamInitials(game.away_team)}</Text>
                  <Text style={[styles.statsHeaderValue, { color: theme.colors.onSurfaceVariant }]}>{getNBATeamInitials(game.home_team)}</Text>
                </View>
                ...
                {(game.away_ats_pct !== null || game.home_ats_pct !== null) && (
                  <View style={styles.statsRow}>
                    <Text style={[styles.statsLabel, { color: theme.colors.onSurfaceVariant }]}>ATS %</Text>
                    <Text style={[styles.statsValue, { color: theme.colors.onSurface }]}>{game.away_ats_pct !== null ? `${(game.away_ats_pct * 100).toFixed(0)}%` : '-'}</Text>
                    <Text style={[styles.statsValue, { color: theme.colors.onSurface }]}>{game.home_ats_pct !== null ? `${(game.home_ats_pct * 100).toFixed(0)}%` : '-'}</Text>
                  </View>
                )}
                {(game.away_over_pct !== null || game.home_over_pct !== null) && (
                  <View style={styles.statsRow}>
                    <Text style={[styles.statsLabel, { color: theme.colors.onSurfaceVariant }]}>Over %</Text>
                    <Text style={[styles.statsValue, { color: theme.colors.onSurface }]}>{game.away_over_pct !== null ? `${(game.away_over_pct * 100).toFixed(0)}%` : '-'}</Text>
                    <Text style={[styles.statsValue, { color: theme.colors.onSurface }]}>{game.home_over_pct !== null ? `${(game.home_over_pct * 100).toFixed(0)}%` : '-'}</Text>
                  </View>
                )}
              </View>
              </View>
              ...
              {!simulationRevealed ? (
                ...
                <View style={[styles.simulationResult, { backgroundColor: 'rgba(251, 191, 36, 0.15)', borderColor: 'rgba(251, 191, 36, 0.3)' }]}>
                  ...
                  <View style={styles.teamScoreSection}>
                    <TeamAvatar teamName={game.away_team} sport="nba" size={64} />
                  ...
                  {/* Home Team Score */}
                  <View style={styles.teamScoreSection}>
                    <TeamAvatar teamName={game.home_team} sport="nba" size={64} />
                    <Text style={[styles.predictedScore, { color: theme.colors.onSurface }]}>
...
const styles = StyleSheet.create({
  contentContainer: { paddingHorizontal: 16, paddingBottom: 40, gap: 12 },
  sectionCard: { borderRadius: 16, borderWidth: 1, padding: 16, overflow: 'hidden' },
  headerGradient: { position: 'absolute', top: 0, left: 0, right: 0, height: 4 },
  headerContent: {},
  sectionContent: {},
  dateTimeRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  ...
  scoreTeamInitials: { fontSize: 18, fontWeight: 'bold', textAlign: 'center' },
