...
import { NCAABSituationalTrendRow, parseNCAABRecord, formatNCAABSituation } from '@/types/ncaabBettingTrends';
import { useThemeContext } from '@/contexts/ThemeContext';
import { TeamAvatar } from '../TeamAvatar';

type SituationType = 'lastGame' | 'favDog' | 'sideFavDog' | 'restBucket' | 'restComp';

interface NCAABTrendsSituationSectionProps {
  title: string;
  icon: string;
 ...
 */
function getATSData(team: NCAABSituationalTrendRow, situationType: SituationType): { record: string; pct: number | null; label: string } {
  switch (situationType) {
    case 'lastGame':
      return {
        record: team.ats_last_game_record || '-',
        pct: team.ats_last_game_cover_pct,
        label: formatNCAABSituation(team.last_game_situation),
      };
    case 'favDog':
      return {
        record: team.ats_fav_dog_record || '-',
        pct: team.ats_fav_dog_cover_pct,
        label: formatNCAABSituation(team.fav_dog_situation),
      };
    case 'sideFavDog':
      return {
        record: team.ats_side_fav_dog_record || '-',
        pct: team.ats_side_fav_dog_cover_pct,
        label: formatNCAABSituation(team.side_spread_situation),
      };
    case 'restBucket':
      return {
        record: team.ats_rest_bucket_record || '-',
        pct: team.ats_rest_bucket_cover_pct,
        label: formatNCAABSituation(team.rest_bucket),
      };
    case 'restComp':
      return {
        record: team.ats_rest_comp_record || '-',
        pct: team.ats_rest_comp_cover_pct,
        label: formatNCAABSituation(team.rest_comp),
      };
  }
}
 ...
 */
function getOUData(team: NCAABSituationalTrendRow, situationType: SituationType): { record: string; overPct: number | null; underPct: number | null } {
  switch (situationType) {
    case 'lastGame':
      return {
 ...
 */
export function NCAABTrendsSituationSection({
  title,
  icon,
  awayTeam,
  homeTeam,
  situationType,
  tooltip,
}: NCAABTrendsSituationSectionProps) {
      ...
      <View style={[styles.content, { backgroundColor: isDark ? 'rgba(255, 255, 255, 0.03)' : 'rgba(0, 0, 0, 0.02)' }]}>
        ...
        <View style={styles.teamHeaderRow}>
          ...
          <View style={styles.teamHeader}>
            <TeamAvatar teamName={awayTeam.team_name} sport="ncaab" size={40} teamAbbr={awayTeam.team_abbr} />
            <Text
          ...
          <View style={styles.teamHeader}>
            <TeamAvatar teamName={homeTeam.team_name} sport="ncaab" size={40} teamAbbr={homeTeam.team_abbr} />
            <Text
              style={[styles.situationLabel, { color: theme.colors.onSurfaceVariant }]}
              numberOfLines={2}
              adjustsFontSizeToFit
