...
## AVOIDING REPETITION
If \`previouslySentSuggestions\` is provided in the context, these are suggestions the user has ALREADY SEEN recently. You MUST:
1. Provide a DIFFERENT insight than any in this list
2. Focus on different games, different bet types, or different angles
3. If all obvious insights were already shown, dig deeper into secondary opportunities
4. Never repeat the same core recommendation, even if phrased differently

Example: If previouslySentSuggestions contains "Chiefs -3 looks strong", don't recommend Chiefs spread again - find a different game or bet type.
...
export default async function wagerBotSuggestions(
  { pageType, sport, data, previousInsight, previousInsights, previouslySentSuggestions, model },
  { auth, logging }
) {
  ...
  // Add suggestion history for deduplication (last 10 suggestions shown to user)
  if (previouslySentSuggestions && previouslySentSuggestions.length > 0) {
    context.previouslySentSuggestions = previouslySentSuggestions;
    logging.log(`  - Previous suggestions to avoid: ${previouslySentSuggestions.length}`);
  }
