...
export default function SettingsScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: pageBackground }]}>
      {/* Native iOS large-title header — mirrors Honeydew's SettingsView
          (`.navigationTitle("Settings") + .navigationBarTitleDisplayMode(.large)`).
          The system handles the back chevron, the title shrink-on-scroll
          transition, the blur-on-scroll backdrop, and accessibility traits.
          The previous custom header (back button + bold "Settings" + subtitle)
          is gone — its job moves to the native chrome + a quiet section
          intro inside the scroll content. */}
      <Stack.Screen
        options={{
          ...
          headerLargeStyle: { backgroundColor: pageBackground },
          headerStyle: { backgroundColor: pageBackground },
        }}
      />
      <ScrollView
        showsVerticalScrollIndicator={false}
        // `contentInsetAdjustmentBehavior="automatic"` is what makes
        // headerLargeTitle's shrink-on-scroll work without us computing the
        // header height by hand. Drop the manual `paddingTop: insets.top + 8`.
