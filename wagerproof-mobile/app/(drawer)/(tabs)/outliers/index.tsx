...
export default function OutliersScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? '#000000' : '#ffffff' }]}>
      {/* Native nav-bar chrome — Honeydew MainFeedsView pattern.
          Title "Alerts", leading cog → settings, trailing robot → WagerBot. */}
      <Stack.Screen
        options={{
          headerTitle: 'Alerts',
      ...
      {/* Inner category switcher — native UISegmentedControl via
          @expo/ui Picker. Replaces the custom underlined inner-tab row.
          Hidden inside the outliers detail view to match the original
          conditional render. */}
      {!(activeTab === 'outliers' && selectedCategory !== null) && (
        <View style={styles.segmentedContainer}>
          <Host matchContents>
            <Picker
              variant="segmented"
              options={INNER_TABS.map((t) => t.label)}
              selectedIndex={Math.max(0, INNER_TABS.findIndex((t) => t.key === activeTab))}
              onOptionSelected={({ nativeEvent: { index } }) => {
                const next = INNER_TABS[index];
...
const styles = StyleSheet.create({
  ...
  // Hosts the SwiftUI Picker(segmented) for the inner category switcher.
  segmentedContainer: {
    paddingHorizontal: 16,
    paddingTop: 8,
    paddingBottom: 4,
