...
export default function FeedScreen() {
  ...
  const renderListHeader = (sport: Sport) => (
    <View>
      <View style={[
        ...
        {/* Inline search row removed — handled by native UISearchController
            wired via `headerSearchBarOptions` on the Stack.Screen above.
            Pull down on the games list to reveal the system search field. */}
        <View style={{ flex: 1 }} />
      ...
      {/* MLB Pitcher Matchups Banner */}
      {sport === 'mlb' && !searchTexts.mlb && <MLBPitcherMatchupsBanner />}

      {/* MLB F5 Splits Banner */}
      {sport === 'mlb' && !searchTexts.mlb && <MLBF5SplitsBanner />}
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? '#000000' : '#ffffff' }]}>
        {/* Native iOS chrome — mirrors Honeydew MainAllRecipesView's
            `.navigationTitle("All Recipes") + .toolbar { ... }`. The system
            paints the large title "WagerProof", the leading cog (settings),
            and the trailing robot (WagerBot manual menu) as real
            UIBarButtonItems. On iOS 26 each toolbar item gets its own
            Liquid Glass capsule for free. Replaces the hand-rolled BlurView
            + ScrollView header that lived here before. */}
        <Stack.Screen
          options={{
            headerTitle: 'WagerProof',
            ...
            headerRight: user
                  ...
                  <View style={styles.toolbarGroup}>
                    {testModeEnabled && (
                      <TouchableOpacity
                        ...
                        accessibilityLabel="Trigger test suggestion"
                        hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
                      >
                        <MaterialCommunityIcons name="lightning-bolt" size={22} color="#00E676" />
                      </TouchableOpacity>
                    )}
                    <TouchableOpacity
              ...
              : undefined,
            // Native UISearchController — pulls down from the nav bar with a
            // proper system search experience (auto cancel button, keyboard
            // handling, voice search, scope bar slot if we ever need filters).
            // Replaces the in-content `<TextInput>` "Search teams or cities…"
            // row that lived inside `renderListHeader`. Mirrors Honeydew's
            // `.searchable($query)` on MainAllRecipesView.
            headerSearchBarOptions: {
              ...
              headerIconColor: 'rgba(255,255,255,0.6)',
              onChangeText: (event) => {
                const text = event.nativeEvent.text;
                setSearchTexts((prev) => ({ ...prev, [selectedSport]: text }));
              },
              onCancelButtonPress: () => {
                setSearchTexts((prev) => ({ ...prev, [selectedSport]: '' }));
              },
            },
          }}
        />

        {/* Native segmented sport switcher (UISegmentedControl via @expo/ui
            Picker `variant: 'segmented'`, hosted in a SwiftUI Host).
            Replaces the custom underlined horizontal scroll row. Beta.9
            uses an `options` string array + `selectedIndex` int instead of
            the children/tag pattern from canary builds. */}
        <View style={styles.segmentedContainer}>
          <Host matchContents>
            <Picker
              variant="segmented"
              options={sports.map((s) => s.label)}
              selectedIndex={Math.max(0, sports.findIndex((s) => s.id === selectedSport))}
              onOptionSelected={({ nativeEvent: { index } }) => {
                const next = sports[index];
                if (!next) return;
                if (next.available) {
                  ...
                  Alert.alert(
                    'Coming Soon',
                    `${next.label} predictions are coming soon! Stay tuned for updates.`,
                    [{ text: 'OK' }]
                  );
                }
              }}
...
const styles = StyleSheet.create({
  ...
  // Right-aligned group of nav-bar icons (test button + WagerBot). Sits in
  // the headerRight slot — on iOS 26 each child renders inside its own
  // Liquid Glass capsule automatically.
  toolbarGroup: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 16,
  },
  // Hosts the SwiftUI segmented picker. Native control measures itself via
  // `matchContents`; we just give it horizontal breathing room.
  segmentedContainer: {
