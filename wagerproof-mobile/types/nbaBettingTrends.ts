/**
 * Types for NBA Situational Betting Trends
 * Data from nba_game_situational_trends_today table
 */

/**
 * Raw row from the nba_game_situational_trends_today table
 * Matches the actual database schema
 */
export interface SituationalTrendRow {
  game_id: number;
  game_date: string;
  team_id: number;
  team_abbr: string;
 ...
 */
export interface NBAGameTrendsData {
  gameId: number;
  gameDate: string;
 ...
 */
export function parseRecord(record: string | null | undefined): { wins: number; losses: number; pushes: number; total: number } {
...
export type TrendsSortMode = 'time' | 'ou-consensus' | 'ats-dominance';
