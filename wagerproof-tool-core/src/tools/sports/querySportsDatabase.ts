...
export const querySportsDatabase: Tool = {
  ...
  description: [
    ...
    "- nfl_betting_lines, nfl_epa_data, nfl_predictions_epa, nfl_team_trends; cfb_games.",
    "- HISTORICAL BETTING LINES (this data EXISTS — never tell users it doesn't): nfl_historical_odds",
    "  (game lines back to 2020), ncaaf_odds_history, cbb_team_lines_history, and the per-sport",
    "  *_odds_snapshots tables for intraday line movement.",
    ...
    "  NFL: nfl_player_props_current (the live board — latest line per player/market/book for this",
    "  week's games), nfl_player_prop_lines_history (every snapshot, all books, since 2023 — use for",
    "  historical prop lines and closing-line research), nfl_player_props (graded rows: actual_value +",
    "  over/under result), nfl_slate_props, nfl_player_prop_trends.",
    "  nfl_prop_player_pages is the NFL equivalent of a ranked board and the FIRST stop for \"what do",
    "  we think about <player>'s <market>\": one row per player per week with the posted markets, our",
    "  `projection` per market, and `research.<market>` carrying the Fantasy-Points prop model",
    "  (pred / line / signed edge / validated threshold / fires) plus the Player Prop Report read with",
    "  its for-and-against tells. `research` is sparse by design — only the markets that backtested",
    "  (pass yds/tds/completions, receptions, reception yds) have it; rushing and anytime TD were",
    "  tested and dropped, so its absence is expected, not missing data.",
    "  For MLB the curated get_player_props tool is faster than SQL for \"what do we like today\".",
    "- CHARTED NFL PLAYER + TEAM DATA — fp_data holds the Fantasy Points Data Suite, 2021-present,",
    "  ~1.6M rows across 28 tools: route running and separation by alignment/coverage/break, man vs",
    "  zone, target and route share, snap share, air yards and passing depth, bell-cow rushing usage,",
    "  pass-rate-over-expected, coverage matchups, per-book fantasy scoring. One row per player-game",
    "  or team-game per (tool, scope) — scope is player | team (that team's offense) | opponent",
    "  (the defense it faced). This is charting you cannot get from box scores.",
    "  Its stats sit in a `stats` jsonb whose key names are long and NOT guessable",
    "  (playerStatsFantasyPointsHalfPpr, marketShareReceivingYardsTotal). ALWAYS query fp_data_keys",
    "  first — one row per (tool, scope, stat_key) with row counts, seasons, type and a sample value;",
    "  add `where is_measure` to skip the *Label / *GamesPlayed bookkeeping keys. Then cast, e.g.",
    "  (stats->>'<key>')::numeric. Guessing a key silently returns zero rows, which looks like",
    "  'no data' rather than a wrong key.",
    "  A stat_key shaped `bucket.<bucketName>.<field>` lives TWO levels down and is read as",
    "  stats->'bucket'->'<bucketName>'->>'<field>' — that is where the by-coverage / by-alignment /",
    "  man-vs-zone splits actually are. Rate fields are 0-1 fractions, not percentages.",
    "",
