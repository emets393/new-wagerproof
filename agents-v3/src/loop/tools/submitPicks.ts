...
import { GeneratedPickV3Schema } from "../pickSchemaV3";
import { clampUnits } from "../unitBands";
import { ensureFormattedGameSnapshot, normalizeDecisionTrace } from "../../shared/agentGameHelpers";
import { getMaxPicks } from "../../generate-avatar-picks/promptBuilder";
...
export async function submitPicks(
  ...
  for (const raw of limited) {
    ...
    // team_total, or any straight the user disallowed). Skipped when the steering
    // has no allowlist (legacy). See plan D2.
  ...
  if (!ctx.dryRun && picksToInsert.length > 0) {
    ...
    if (straightRows.length > 0) {
      ...
      if (error) {
        // Surface, don't just swallow: a silent DB error here makes the run
        // report success with picks_generated=N while 0 rows actually persist.
        console.error(`[v3 submit] straight upsert failed (${straightRows.length} rows): ${error.message} | details=${(error as { details?: string }).details ?? ""}`);
        report.ok = false;
    ...
    if (propRows.length > 0) {
      ...
      if (propError) {
        console.error(`[v3 submit] prop insert failed (${propRows.length} rows): ${propError.message} | details=${(propError as { details?: string }).details ?? ""}`);
        report.ok = false;
