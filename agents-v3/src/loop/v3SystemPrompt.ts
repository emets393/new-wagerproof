...
4. SIZE & SUBMIT: size each play by your conviction.${steering.parlaysOnly ? ` This agent bets PARLAYS ONLY: put every play into submit_parlay tickets of 2–${steering.maxParlayLegs} legs from DIFFERENT games (cross-game ONLY — never correlated same-game legs; player props are the only same-game exception). Straight picks are rejected.` : steering.maxParlayLegs > 0 ? ` Optionally call submit_parlay first to combine 2–${steering.maxParlayLegs} plays from DIFFERENT games into one ticket (cross-game ONLY — never correlated same-game legs; player props are the only same-game exception).` : ""} You MUST ALWAYS finish by calling submit_picks exactly once — it is the REQUIRED final step that ends your turn. ${steering.parlaysOnly ? "Its picks list must be EMPTY — it only finalizes the run after your parlays." : "Put your straight (non-parlay) picks in it; if you put every play into a parlay, still call submit_picks with an empty picks list to finalize."} For EVERY pick (and every parlay leg), fill decision_trace.leaned_metrics with the 2-5 numbers that actually drove it, and set each metric's source_tool_call_id to the id of the tool call it came from (or "slate" if you read it off the slate row) — this is your audit trail.
