export default async function ({ pickData, gameData, channelId, botToken }) {
  ...
  const getSportConfig = (gameType) => {
    ...
    return configs[gameType] || { emoji: '🏈', label: 'PICK' };
  };
  ...
  try {
    ...
    // Build formatted bets - NEW LOGIC
    // If pickValue exists, use it with additional details
    // Otherwise fall back to the old formatBetType logic
    let formattedBets;
    if (pickData.pickValue) {
      // New format with manual pick value
      let pickLine = `🎯 **${pickData.pickValue}**`;
      
      // Add price if available
      if (pickData.bestPrice) {
        pickLine += ` @ ${pickData.bestPrice}`;
      }
      
      // Add sportsbook if available
      if (pickData.sportsbook) {
        pickLine += ` (${pickData.sportsbook})`;
      }
      
      // Add units if available
      if (pickData.units) {
        const unitText = pickData.units === 1 ? 'unit' : 'units';
        pickLine += ` - ${pickData.units} ${unitText}`;
      }
      
      formattedBets = pickLine;
    } else {
      // Old format for backward compatibility
      formattedBets = pickData.selectedBetTypes
    ...
    const payload = {
      embeds: [{
        ...
        footer: {
          text: `WagerBot • Editor's Pick • ${sportConfig.label}`,
