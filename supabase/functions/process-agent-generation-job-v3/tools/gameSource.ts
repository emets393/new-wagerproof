...
import { getDateTimeInET, footballWeekKeyForDate } from "../../shared/dateUtils.ts";
...
export async function loadGames(ctx: AgentGenContext): Promise<{ total: number; bySport: { sport: Sport; count: number }[] }> {
  ...
  for (const sport of ctx.steering.preferredSports as Sport[]) {
    try {
      ...
      // Offer only the EARLIEST Tue-anchored football week with games — owner
      // rule 2026-08-27; mirrors agents-v3/src/loop/tools/gameSource.ts.
