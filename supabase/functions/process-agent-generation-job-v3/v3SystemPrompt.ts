...
4. SIZE & SUBMIT: size each play by your conviction.${steering.maxParlayLegs > 0 ? ` Optionally call submit_parlay first to combine 2–${steering.maxParlayLegs} plays from DIFFERENT games into one ticket (cross-game ONLY — never correlated same-game legs; player props are the only same-game exception, EXCEPT the volume markets player_pass_attempts / player_rush_attempts / player_pass_completions, which reflect the whole game script and must be the ONLY leg from their game — never pair one with any other pick from the same game).` : ""} You MUST ALWAYS finish by calling submit_picks exactly once — it is the REQUIRED final step that ends your turn. Put your straight (non-parlay) picks in it; if you put every play into a parlay, still call submit_picks with an empty picks list to finalize. For EVERY pick (and every parlay leg), fill decision_trace.leaned_metrics with the 2-5 numbers that actually drove it, and set each metric's source_tool_call_id to the id of the tool call it came from (or "slate" if you read it off the slate row) — this is your audit trail.
