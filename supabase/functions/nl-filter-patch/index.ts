...
// code the web runs client-side, so server-validated results can never drift from web behavior.
import { applyFilterPatch, normalizeNflSavedFilterSnapshot } from './reducer.gen.mjs';
...
serve(async (req) => {
  ...
  try {
    ...
    if (body?.apply === true) {
      const current = normalizeNflSavedFilterSnapshot(currentFilter);
      const result = applyFilterPatch(current, { ops }, { coaches, referees });
      return json({
