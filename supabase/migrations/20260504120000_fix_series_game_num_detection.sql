-- Fix the G2 vs G3 vs G4+ detection in mlb_game_signals.
--
-- BUG: the prior view fired G3-tagged "carryover" signals on EVERY game
-- where the team had 2+ prior games against the same opponent in the
-- last 6 days — including G4 of a 4-game series. On 2026-05-04 the
-- NYY/BAL game (G4) showed "won G2 by 8, momentum extends to G3" when
-- in reality it was G3 that finished 11-3 and today is G4.
--
-- ROOT CAUSE: the trigger conditions used `g2_margin IS NULL` for G2-day
-- and `g2_margin IS NOT NULL` for G3-day. For G4+ the 2-back game still
-- exists (it's just G2 of the series), so all "G3 signal" branches
-- fired with last-game-margin = G3 actual margin and 2-back margin =
-- G2 actual margin, mislabeled as G2 and G1.
--
-- FIX:
--   1. Add g3_for_home / g3_for_away CTEs that fetch the 3-back game.
--   2. Each G3-signal trigger now also requires `g3_margin IS NULL`
--      (= no 3-back game = today is exactly G3, not G4+).
--   3. G2 triggers stay as-is (`g2_margin IS NULL` already excludes G3+).
--   4. G4+ days produce no series carryover signal at all — historical
--      samples are too thin and patterns invert vs G3.
--
-- Everything else in the view (pitcher / bullpen / batting / schedule /
-- weather / park signals) is unchanged.
...
-- NEW: 3-back game (only present on G4+ day). Used as a hard guard so
-- the G3 series triggers below stop firing once we're past G3.
g3_for_home AS (
  SELECT today.game_pk AS today_game_pk,
         g3h.margin AS g3_margin
  FROM today_games today
  LEFT JOIN LATERAL (
    SELECT gl.margin
    FROM public.mlb_game_log gl
...
SELECT g.game_pk, g.official_date, g.home_team_name, g.away_team_name,
  ...
  array_remove(ARRAY[
    ...
    -- G2 SERIES (today is exactly G2 — no 2-back game) — unchanged
    ...
    -- G3 SERIES (today is exactly G3 — 2-back exists, 3-back does NOT)
    -- BUG FIX: every trigger now requires `g3h.g3_margin IS NULL`.
    CASE WHEN g2h.g2_margin IS NOT NULL AND g3h.g3_margin IS NULL AND g1h.g1_margin >= 15
         THEN json_build_object('category','series','severity','negative','message',
    ...
    CASE WHEN g2h.g2_margin IS NOT NULL AND g3h.g3_margin IS NULL AND g1h.g1_margin >= 8 AND g1h.g1_margin < 15 AND g1h.g1_ml BETWEEN -149 AND -110
         THEN json_build_object('category','series','severity','negative','message',
    ...
    CASE WHEN g2h.g2_margin IS NOT NULL AND g3h.g3_margin IS NULL AND g1h.g1_margin >= 8 AND g1h.g1_margin < 15 AND g1h.g1_ml <= -150
         THEN json_build_object('category','series','severity','positive','message',
  ...
  array_remove(ARRAY[
    ...
    -- AWAY G3 SERIES — every trigger now requires `g3a.g3_margin IS NULL`
    CASE WHEN g2a.g2_margin IS NOT NULL AND g3a.g3_margin IS NULL AND g1a.g1_margin >= 15
         THEN json_build_object('category','series','severity','negative','message',
    ...
    CASE WHEN g2a.g2_margin IS NOT NULL AND g3a.g3_margin IS NULL AND g1a.g1_margin >= 8 AND g1a.g1_margin < 15 AND g1a.g1_ml BETWEEN -149 AND -110
         THEN json_build_object('category','series','severity','negative','message',
    ...
    CASE WHEN g2a.g2_margin IS NOT NULL AND g3a.g3_margin IS NULL AND g1a.g1_margin >= 8 AND g1a.g1_margin < 15 AND g1a.g1_ml <= -150
         THEN json_build_object('category','series','severity','positive','message',
...
COMMENT ON VIEW public.mlb_game_signals IS
  'Daily MLB pre-game signals view. G2 series carryover signals fire only '
  'when today is exactly G2 (no 2-back game vs same opponent). G3 series '
  'signals fire only when today is exactly G3 (2-back exists, 3-back does '
  'not). G4+ days produce no series carryover signal — historical samples '
  'are too thin and patterns invert vs G3. Bug fixed in migration '
  '20260504120000_fix_series_game_num_detection.sql.';
